...
class TestDeerFlowBashLoop:
    ...
    def _build_deer_flow_transcript(self, iterations: int) -> list[dict[str, Any]]:
        ...
        for i in range(iterations):
            ...
            messages.append(
                _make_tool_call_msg(
                    call_id, "bash", {"command": "ls /nonexistent/config"}
                )
            )  # noqa: E501
            messages.append(
    ...
    def test_escalation_through_all_stages(self):
        ...
        for turn in range(1, 10):
            ...
            decision = engine.process(
                messages, state, thread_id="deer-flow-test", node_name="agent"
            )  # noqa: E501
        ...
        first_nudge = next(
            i for i, s in enumerate(stages_seen) if s == InterventionStage.NUDGE
        )  # noqa: E501
        first_override = next(
            i for i, s in enumerate(stages_seen) if s == InterventionStage.OVERRIDE
        )  # noqa: E501
        first_stop = next(
            i for i, s in enumerate(stages_seen) if s == InterventionStage.HARD_STOP
        )  # noqa: E501
        assert first_nudge < first_override < first_stop, (
            "Stages must escalate monotonically"
        )  # noqa: E501
    ...
    def test_nudge_contains_coaching_for_bash(self):
        ...
        for turn in range(1, 5):
            ...
            decision = engine.process(
                messages, state, thread_id="t1", node_name="agent"
            )  # noqa: E501
            for k, v in decision.state_patch.items():
                tc_state[k] = v
            if decision.stage == InterventionStage.NUDGE:
                ...
                assert (
                    "approach" in last_msg["content"].lower()
                    or "different" in last_msg["content"].lower()
                )  # noqa: E501
                return
    ...
    def test_override_strips_repeated_bash_calls(self):
        ...
        config = InterventionConfig(
            nudge_threshold=2, override_threshold=3, hard_stop_threshold=6
        )  # noqa: E501
        engine = InterventionEngine(config=config)
        tc_state = default_intervention_state()
        ...
        for turn in range(1, 8):
            ...
            decision = engine.process(
                messages, state, thread_id="t2", node_name="agent"
            )  # noqa: E501
            for k, v in decision.state_patch.items():
                tc_state[k] = v
        ...
        pytest.fail(
            f"Should have reached OVERRIDE within 7 turns, last stage: {decision.stage.name}"
        )  # noqa: E501
    ...
    def test_hard_stop_terminates(self):
        ...
        for turn in range(1, 15):
            ...
            decision = engine.process(
                messages, state, thread_id="t3", node_name="agent"
            )  # noqa: E501
            for k, v in decision.state_patch.items():
                tc_state[k] = v
            if decision.should_terminate:
        ...
        pytest.fail(
            f"Should have reached HARD_STOP within 14 turns, last stage: {decision.stage.name}"
        )  # noqa: E501
...
class TestHermesGrepLoop:
    ...
    def _build_hermes_transcript(self, iterations: int) -> list[dict[str, Any]]:
        ...
        for i in range(iterations):
            ...
            messages.append(
                _make_tool_call_msg(
                    call_id, "grep", {"pattern": pattern, "path": "./src"}
                )
            )  # noqa: E501
    ...
    def test_hermes_loop_detected_despite_variations(self):
        ...
        for turn in range(1, 10):
            ...
            state = {
                "messages": messages,
                "_tc_intervention": dict(tc_state),
                "configurable": {"thread_id": "hermes"},
            }  # noqa: E501
            decision = engine.process(
                messages, state, thread_id="hermes", node_name="agent"
            )  # noqa: E501
            for k, v in decision.state_patch.items():
                tc_state[k] = v
    ...
    def test_hermes_override_preserves_original_task(self):
        ...
        if decision.llm_input_messages:
            contents = " ".join(
                m.get("content", "") or "" for m in decision.llm_input_messages
            )  # noqa: E501
...
class TestMixedToolLoop:
    ...
    def _build_mixed_loop(self, iterations: int) -> list[dict[str, Any]]:
        ...
        for i in range(iterations):
            if i % 2 == 0:
                ...
                messages.append(
                    _make_tool_call_msg(
                        call_id, "web_search", {"query": "library X release notes"}
                    )
                )  # noqa: E501
                messages.append(
                    _make_tool_result_msg(
                        call_id, "No relevant results found.", "web_search"
                    )
                )  # noqa: E501
            else:
                ...
                messages.append(
                    _make_tool_call_msg(
                        call_id, "url_fetch", {"url": "https://lib-x.io/releases"}
                    )
                )  # noqa: E501
                messages.append(
                    _make_tool_result_msg(
                        call_id, "Error 404: Page not found", "url_fetch"
                    )
                )  # noqa: E501
        return messages
    ...
    def test_mixed_tool_loop_eventually_intervenes(self):
        ...
        for turn in range(1, 12):
            ...
            decision = engine.process(
                messages, state, thread_id="mixed", node_name="agent"
            )  # noqa: E501
            for k, v in decision.state_patch.items():
                tc_state[k] = v
    ...
    def test_progress_resets_intervention(self):
        ...
        state = {
            "messages": stagnant_messages,
            "_tc_intervention": dict(tc_state),
            "configurable": {"thread_id": "prog"},
        }  # noqa: E501
        for turn in range(1, 4):
            decision = engine.process(
                stagnant_messages, state, thread_id="prog", node_name="agent"
            )  # noqa: E501
            for k, v in decision.state_patch.items():
                tc_state[k] = v
        ...
        progress_messages = list(stagnant_messages) + [
            ...
            {
                "role": "assistant",
                "content": "I found the release notes! Version 2.0 includes Feature A and Feature B.",
            },  # noqa: E501
        ]
        state_progress = {
        ...
        decision = engine.process(
            progress_messages, state_progress, thread_id="prog", node_name="agent"
        )  # noqa: E501
