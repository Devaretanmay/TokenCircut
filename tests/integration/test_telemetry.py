...
def test_engine_emits_otel_spans():
    ...
    with patch("tokencircuit.engine._get_tracer", return_value=mock_tracer):
        config = InterventionConfig(max_tokens_per_turn=10) # Set low to force RUNAWAY_GENERATION  # noqa: E501
        engine = InterventionEngine(config=config)

        messages = [
            HumanMessage(content="Test"),
            AIMessage(content="A" * 100), # > 10 tokens -> RUNAWAY_GENERATION -> HARD_STOP  # noqa: E501
        ]
        ...
        mock_span.add_event.assert_any_call("SignalDetected", {"signal.type": SignalType.RUNAWAY_GENERATION.value})  # noqa: E501
