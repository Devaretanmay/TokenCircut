"""
State Isolation Tests — verify _tc_intervention is strictly isolated per (thread_id, node_name).
...
def _stagnant_messages() -> list[dict[str, Any]]:
    ...
    return [
        ...
        {"role": "tool", "content": "Not found", "tool_call_id": "call_1", "name": "search"},
    ]
...
def _progress_messages() -> list[dict[str, Any]]:
    ...
    return [
        ...
        {"role": "assistant", "content": "Based on my research, I've found the following key points about the quantum entanglement phenomenon."},
    ]
...
class TestThreadIdIsolation:
    ...
    def test_independent_stagnation_counters(self):
        ...
        for turn in range(1, 5):
            state_a = {"messages": messages, "_tc_intervention": dict(tc_state_a), "configurable": {"thread_id": "thread_a"}}
            decision_a = engine.process(messages, state_a, thread_id="thread_a", node_name="agent")
            for k, v in decision_a.state_patch.items():
                tc_state_a[k] = v

        # thread_b should start fresh
        tc_state_b = default_intervention_state()
        state_b = {"messages": messages, "_tc_intervention": tc_state_b, "configurable": {"thread_id": "thread_b"}}
        decision_b = engine.process(messages, state_b, thread_id="thread_b", node_name="agent")

        assert decision_b.stage == InterventionStage.PASS, (
            f"Thread B should start at PASS, got {decision_b.stage.name} "
            f"(leaked from thread A which is at stagnation count {tc_state_a.get('consecutive_stagnation_count')})"
        )

    def test_100_independent_threads(self):
        """100 concurrent threads should have completely independent states."""
        config = InterventionConfig(nudge_threshold=3, override_threshold=5, hard_stop_threshold=8)
        engine = InterventionEngine(config=config)
        ...
        for thread_idx in range(100):
            ...
            for turn in range(num_turns):
                state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": thread_id}}
                decision = engine.process(messages, state, thread_id=thread_id, node_name="agent")
        ...
        for tid, stages in results.items():
            if len(stages) == 1:
                assert stages[0] == InterventionStage.PASS, f"{tid}: first turn should always be PASS"
...
class TestNodeNameIsolation:
    ...
    def test_agent_and_reviewer_independent(self):
        ...
        for turn in range(1, 5):
            state = {"messages": stagnant, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "t1"}}
            decision = engine.process(stagnant, state, thread_id="t1", node_name="agent")

        # "reviewer" in same thread should be independent
        state_r = {"messages": progress, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "t1"}}
        decision_r = engine.process(progress, state_r, thread_id="t1", node_name="reviewer")

        assert decision_r.stage == InterventionStage.PASS, (
            f"reviewer should be PASS (independent of agent), got {decision_r.stage.name}"
        )

    def test_three_nodes_independent_escalation(self):
        """Three nodes can be at different escalation stages simultaneously."""
        config = InterventionConfig(nudge_threshold=2, override_threshold=3, hard_stop_threshold=5)
        engine = InterventionEngine(config=config)
        messages = _stagnant_messages()
        ...
        for node_name, num_turns in [("agent", 5), ("planner", 3), ("critic", 1)]:
            tc_state = default_intervention_state()
            for turn in range(num_turns):
                state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "shared"}}
                decision = engine.process(messages, state, thread_id="shared", node_name=node_name)
                for k, v in decision.state_patch.items():
                    tc_state[k] = v
        ...
        assert node_stages["planner"] >= InterventionStage.NUDGE  # 3 turns >= nudge_threshold
        assert node_stages["agent"] >= InterventionStage.OVERRIDE  # 5 turns >= override_threshold
...
class TestConcurrentExecution:
    ...
    def test_concurrent_threads_no_corruption(self):
        ...
        def worker(thread_id: str, num_turns: int) -> tuple[str, list[InterventionStage]]:
            stages: list[InterventionStage] = []
            tc_state = default_intervention_state()
            try:
                for turn in range(num_turns):
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = []
            for i in range(20):
                futures.append(executor.submit(worker, f"concurrent_{i}", turns := (i % 6) + 2))
    ...
    def test_concurrent_same_thread_different_nodes(self):
        ...
        def process_node(node_name: str, turns: int) -> InterventionStage:
            ...
            for turn in range(turns):
                state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "shared_thread"}}
                decision = engine.process(messages, state, thread_id="shared_thread", node_name=node_name)
        ...
        assert results["node_0"] <= results["node_3"], (
            f"More turns should escalate further: node_0={results['node_0'].name}, node_3={results['node_3'].name}"
        )
...
class TestStatePersistedAcrossCalls:
    ...
    def test_stagnation_counter_accumulates(self):
        ...
        for turn in range(1, 6):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "acc"}}
            decision = engine.process(messages, state, thread_id="acc", node_name="agent")
            for k, v in decision.state_patch.items():
                tc_state[k] = v
    ...
    def test_stage_persists_in_state_patch(self):
        ...
        for turn in range(1, 5):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "stage"}}
            decision = engine.process(messages, state, thread_id="stage", node_name="agent")
            for k, v in decision.state_patch.items():
                tc_state[k] = v
