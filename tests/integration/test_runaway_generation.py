...
def test_runaway_generation_hard_stops_immediately():
    ...
    decision = engine.process(messages=messages, state=state, thread_id="test", node_name="agent")
...
def test_runaway_generation_respects_audit_mode():
    ...
    # We will test the adapter side indirectly via integration, or rely on existing audit test
