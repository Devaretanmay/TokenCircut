...
def _build_realistic_transcript(turns: int) -> list[dict[str, Any]]:
    ...
    messages: list[dict[str, Any]] = [
        {
            "role": "system",
            "content": "You are an expert coding assistant. " * 20,
        },  # ~100 tokens  # noqa: E501
        {
            "role": "user",
            "content": "Implement a REST API for user management with authentication, "  # noqa: E501
            "database models, and comprehensive error handling. " * 5,
        },  # ~50 tokens
    ]
...
class TestPerformanceBaseline:
    ...
    def test_semantic_detection_under_1ms(self):
        """SemanticStagnationDetector should analyze in < 1ms."""
    ...
    def test_full_pipeline_under_2ms_10_turns(self):
        ...
        for turn in range(100):
            state = {
                "messages": messages,
                "_tc_intervention": dict(tc_state),
                "configurable": {"thread_id": f"perf_{turn}"},
            }  # noqa: E501
            start = time.perf_counter_ns()
    ...
    def test_full_pipeline_under_2ms_20_turns(self):
        ...
        for turn in range(100):
            state = {
                "messages": messages,
                "_tc_intervention": dict(tc_state),
                "configurable": {"thread_id": f"pw_{turn}"},
            }  # noqa: E501
            start = time.perf_counter_ns()
    ...
    def test_full_pipeline_under_3ms_50_turns(self):
        ...
        for turn in range(50):
            state = {
                "messages": messages,
                "_tc_intervention": dict(tc_state),
                "configurable": {"thread_id": f"p50_{turn}"},
            }  # noqa: E501
            start = time.perf_counter_ns()
        ...
        assert p99 < 30000, (
            f"Full pipeline p99 = {p99:.0f}μs EXCEEDS 30ms target for 50-turn transcript"
        )  # noqa: E501
    ...
    def test_nudge_decision_no_significant_overhead(self):
        ...
        for trial in range(50):
            ...
            state = {
                ...
                "configurable": {"thread_id": f"pass_{trial}"},
            }  # noqa: E501
            start = time.perf_counter_ns()
            engine.process(
                messages, state, thread_id=f"pass_{trial}", node_name="agent"
            )  # noqa: E501
        ...
        for trial in range(50):
            ...
            state = {
                ...
                "configurable": {"thread_id": f"nudge_{trial}"},
            }  # noqa: E501
            start = time.perf_counter_ns()
            engine.process(
                messages, state, thread_id=f"nudge_{trial}", node_name="agent"
            )  # noqa: E501
        ...
        assert overhead < 1500, (
            f"NUDGE overhead = {overhead:.0f}μs (should be < 1500μs)"
        )  # noqa: E501
    ...
    def test_ledger_does_not_grow_unbounded(self):
        ...
        for turn in range(100):
            state = {
                "messages": messages,
                "_tc_intervention": dict(tc_state),
                "configurable": {"thread_id": "mem"},
            }  # noqa: E501
            decision = engine.process(
                messages, state, thread_id="mem", node_name="agent"
            )  # noqa: E501
            for k, v in decision.state_patch.items():
                tc_state[k] = v

        # Check internal state isn't huge
        ts = engine._thread_states.get("mem:agent")
        assert ts is None or ts.ledger.total_committed <= 500, (
            "Ledger growing unbounded"
        )
    ...
    def test_detector_window_capped(self):
        ...
        for turn in range(50):
            state = {
                "messages": messages,
                "_tc_intervention": dict(tc_state),
                "configurable": {"thread_id": "win"},
            }  # noqa: E501
            decision = engine.process(
                messages, state, thread_id="win", node_name="agent"
            )  # noqa: E501
            for k, v in decision.state_patch.items():
                tc_state[k] = v

        ts = engine._thread_states.get("win:agent")
        assert ts is not None
        assert ts.detector is None or len(ts.detector._window) <= 5
