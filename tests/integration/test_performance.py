...
Target: < 2ms per hook invocation (canonicalization + validation +
shingling + decision).
Tests use real-world-sized message transcripts to ensure the benchmark
is representative.
"""
...
def _build_realistic_transcript(turns: int) -> list[dict[str, Any]]:
    ...
    messages: list[dict[str, Any]] = [
        {"role": "system", "content": "You are an expert coding assistant. " * 20},  # ~100 tokens  # noqa: E501
        {"role": "user", "content": "Implement a REST API for user management with authentication, "  # noqa: E501
    ...
    tool_results = [
        ...
        "Found 3 files matching pattern:\n  src/models/user.py\n  src/routes/auth.py\n  tests/test_user.py",  # noqa: E501
        '{"status": "success", "users": [{"id": 1, "name": "Alice"}, {"id": 2, "name": "Bob"}]}',  # noqa: E501
...
class TestEndToEndHookLatency:
    ...
    def test_full_pipeline_under_2ms_10_turns(self):
        ...
        # Warmup
        for _ in range(5):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "perf"}}  # noqa: E501
        ...
        for turn in range(100):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": f"perf_{turn}"}}  # noqa: E501
            start = time.perf_counter_ns()
            engine.process(messages, state, thread_id=f"perf_{turn}", node_name="agent")  # noqa: E501
            elapsed_us = (time.perf_counter_ns() - start) / 1000
            times.append(elapsed_us)
    ...
    def test_full_pipeline_under_2ms_20_turns(self):
        ...
        # Warmup
        for _ in range(5):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "pw"}}  # noqa: E501
        ...
        times: list[float] = []
        for turn in range(100):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": f"pw_{turn}"}}  # noqa: E501
            start = time.perf_counter_ns()
    ...
    def test_full_pipeline_under_3ms_50_turns(self):
        ...
        # Warmup
        for _ in range(3):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "p50"}}  # noqa: E501
        ...
        for turn in range(50):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": f"p50_{turn}"}}  # noqa: E501
            start = time.perf_counter_ns()
        ...
        assert p99 < 30000, f"Full pipeline p99 = {p99:.0f}μs EXCEEDS 30ms target for 50-turn transcript"  # noqa: E501

    def test_nudge_decision_no_significant_overhead(self):
        """NUDGE decisions should not add significant overhead vs PASS."""
        messages = _build_realistic_transcript(10)
        config = InterventionConfig(nudge_threshold=1, override_threshold=3, hard_stop_threshold=5)  # noqa: E501

        # Measure PASS decisions
        pass_times: list[float] = []
        for trial in range(50):
            engine = InterventionEngine(config=config)
            state = {"messages": messages, "_tc_intervention": default_intervention_state(), "configurable": {"thread_id": f"pass_{trial}"}}  # noqa: E501
            start = time.perf_counter_ns()
            engine.process(messages, state, thread_id=f"pass_{trial}", node_name="agent")  # noqa: E501
        ...
        for trial in range(50):
            ...
            state = {"messages": messages, "_tc_intervention": tc_state, "configurable": {"thread_id": f"nudge_{trial}"}}  # noqa: E501
            start = time.perf_counter_ns()
            engine.process(messages, state, thread_id=f"nudge_{trial}", node_name="agent")  # noqa: E501
        ...
        print(f"\n  PASS p50={pass_p50:.0f}μs, NUDGE p50={nudge_p50:.0f}μs, overhead={overhead:.0f}μs")  # noqa: E501
        # NUDGE should add less than 1500μs overhead
        assert overhead < 1500, f"NUDGE overhead = {overhead:.0f}μs (should be < 1500μs)"  # noqa: E501
...
class TestMemoryEfficiency:
    ...
    def test_ledger_does_not_grow_unbounded(self):
        ...
        tc_state = default_intervention_state()
        for turn in range(100):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "mem"}}  # noqa: E501
            decision = engine.process(messages, state, thread_id="mem", node_name="agent")  # noqa: E501
    ...
    def test_detector_window_capped(self):
        ...
        tc_state = default_intervention_state()
        for turn in range(50):
            state = {"messages": messages, "_tc_intervention": dict(tc_state), "configurable": {"thread_id": "win"}}  # noqa: E501
            decision = engine.process(messages, state, thread_id="win", node_name="agent")  # noqa: E501
            for k, v in decision.state_patch.items():
                tc_state[k] = v
