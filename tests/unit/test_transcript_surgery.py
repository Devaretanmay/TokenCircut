...
def test_override_surgery_compacts_repetitive_tool_calls():
    ...
    """
    engine = InterventionEngine(config=InterventionConfig(nudge_threshold=2, override_threshold=3))
    
    # 1. Build a repetitive transcript
    messages = [
        CanonicalMessage(role=CanonicalRole.SYSTEM, content="You are a helpful assistant."),
        CanonicalMessage(role=CanonicalRole.HUMAN, content="Fetch the data."),
        # Turn 1: Call tool A
        CanonicalMessage(role=CanonicalRole.AI, tool_calls=[{"id": "call_1", "name": "get_data", "args": {}}]),
        CanonicalMessage(role=CanonicalRole.TOOL, tool_call_id="call_1", content="Error: 403"),
        # Turn 2: Repeat call tool A (identical)
        CanonicalMessage(role=CanonicalRole.AI, tool_calls=[{"id": "call_2", "name": "get_data", "args": {}}]),
        CanonicalMessage(role=CanonicalRole.TOOL, tool_call_id="call_2", content="Error: 403"),
        # Turn 3: Repeat call tool A (identical) -> triggers OVERRIDE
        CanonicalMessage(role=CanonicalRole.AI, tool_calls=[{"id": "call_3", "name": "get_data", "args": {}}]),
        CanonicalMessage(role=CanonicalRole.TOOL, tool_call_id="call_3", content="Error: 403"),
    ]
    ...
    # Check compaction:
    # Original: SYSTEM, HUMAN, AI(call_1), TOOL(call_1), AI(call_2), TOOL(call_2), AI(call_3), TOOL(call_3)
